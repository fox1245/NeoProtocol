...
| [`examples/cowork-poc/`](examples/cowork-poc/) | Workspace Stages 1+2+3 | ✅ runnable | **Two browsers, one document, two agents — and the agents can talk, optionally with no vendor cloud at all.** Y.js CRDT over a Workspace data channel + cross-agent ACP over a second multiplexed channel. CodeMirror 6 editor with live remote cursors. Stage 1: each user prompts *their own* agent. Stage 2: a user can *send to peer's agent* with permission gating. Stage 3: agent-mode dropdown supports BYOK (Anthropic), **Local (transformers.js + WebGPU; Gemma / Llama)**, or Mock — both peers can run local models for end-to-end no-cloud cooperation. See [SPEC §17](SPEC.md) and [roadmap](docs/roadmap-collaborative-workspace.md) |
