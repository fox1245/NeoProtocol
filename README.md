...
| [`examples/cowork-poc/`](examples/cowork-poc/) | Workspace Stages 1+2 | ✅ runnable | **Two browsers, one document, two agents — and the agents can talk.** Y.js CRDT over a Workspace data channel + cross-agent ACP over a second multiplexed channel. CodeMirror 6 editor with live remote cursors. Each user has a BYOK (Anthropic) or offline-mock agent. Stage 1: each user prompts *their own* agent, suggested edits propagate with attribution. Stage 2: a user can *send to peer's agent* — peer's UI gates with `allow_once` / `allow_session` / `deny_*`. See [SPEC §17](SPEC.md) and [roadmap](docs/roadmap-collaborative-workspace.md) |
